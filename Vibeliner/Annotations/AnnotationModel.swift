...
struct Annotation: Identifiable {
    ...
    /// Relative position (0.0–1.0) within parent image. For arrows, the end point
    /// is relative to `endImageIndex` (cross-image arrows).
    ...
    /// `nil` when both endpoints are on the same image.
