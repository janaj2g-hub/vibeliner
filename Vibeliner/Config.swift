...
enum PromptBuilder {
    ...
    static let defaultSinglePreamble = "View the screenshot at `\(screenshotPathToken)` — it shows my running app with red numbered annotations indicating issues to fix."
    static let defaultBatchPreamble = "I have multiple screenshots showing issues in my running app. Each screenshot path is included below. Fix all issues across all screenshots."
    ...
    static func buildPrompt(
        ...
        if annotations.isEmpty {
            ...
            for annotation in annotations.sorted(by: { $0.number < $1.number }) {
                let noteText = annotation.note.isEmpty ? "[no description]" : annotation.note
                lines.append("\(annotation.number). \(noteText)")
            }
        }
